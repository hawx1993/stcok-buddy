    ...
    <section className={styles['market-review-card']} data-card>
      <header className={styles['market-review-header']}>
        <div>
          ...
          <span>
            {report.tradeDate ?? '暂无数据'} · {dataSources.length ? dataSources.join(' / ') : '暂无数据'}
          </span>
        </div>
        <div className={styles['sentiment-score']}>
          {report.sentimentScore === null || report.sentimentScore === undefined
            ? '暂不评分'
            : `${report.sentimentScore}分`}
        </div>
      </header>
      <ReviewSection title='📈 市场情绪'>
        <MetricGrid metrics={sentiment} />
      </ReviewSection>
            ...
            return (
              <article className={styles['review-theme']} key={theme.id}>
                <div className={styles['review-theme-heading']}>
                  ...
                  <button
                    className={styles['review-theme-expand']}
                    onClick={() => setExpandedTheme(expanded ? undefined : theme.id)}
                    type='button'
                  >
                    {expanded ? '收起' : '展开'}
                  </button>
                ...
                <div className={styles['review-theme-facts']}>
                  <span className={styles['review-tone-up']}>涨停 {formatValue(theme.limitUpCount, '家')}</span>
                  <span>龙头 {theme.leaderName ?? '暂无数据'}</span>
                  <span className={styles['review-tone-neutral']}>高度 {formatValue(theme.leaderHeight, '板')}</span>
                </div>
                {expanded ? (
                  <div className={styles['review-theme-detail']}>
                    <p>上涨原因：{theme.reason ?? '暂无数据'}</p>
                    <p>资金流向：{formatMoney(theme.mainNetInflow)}</p>
                    <p>板块成交额：{formatMoney(theme.amount)}</p>
                    <p>
                      今日涨停名单：
                      {limitUpStocks.length
                        ? limitUpStocks
                            .map((stock) => `${stock.name}${stock.height ? ` ${stock.height}板` : ''}`)
                            .join('、')
                        : '暂无数据'}
                    </p>
                    <p>
                      核心股：
                      {coreStocks.length
                        ? coreStocks.map((stock) => `${stock.name} ${formatPercent(stock.changePercent)}`).join('、')
                        : '暂无数据'}
                    </p>
      ...
      <ReviewSection title='🏢 龙头股复盘'>
        {leaders.length ? (
          leaders.map((leader) => (
            <article className={styles['review-leader']} key={leader.code}>
              <strong>
                {leader.name} {leader.height ? `${leader.height}连板` : ''}
              </strong>
              <span>所属概念：{leader.concepts.length ? leader.concepts.join('、') : '暂无数据'}</span>
              <span className={styles['review-tone-neutral']}>成交额：{formatMoney(leader.amount)}</span>
              <span className={styles['review-tone-neutral']}>换手：{formatPercent(leader.turnoverRate)}</span>
              <span className={styles['review-tone-up']}>封单：{formatMoney(leader.sealAmount)}</span>
            </article>
          ))
        ) : (
          <EmptyState />
      ...
      <ReviewSection title='📅 明日重点关注'>
        ...
        <ol className={styles['review-focus-list']}>
          {nextDayFocus.length ? (
            nextDayFocus.map((item) => (
              <li className={styles[`review-tone-${item.tone}`]} key={item.id}>
                {item.condition}
              </li>
            ))
          ) : (
...
function MetricGrid({ metrics }: { metrics: IMarketReviewMetric[] }) {
  return (
    <div className={styles['review-metric-grid']}>
      {metrics.map((metric) => (
        <div key={metric.label}>
          <span>{metric.label}</span>
          <strong className={styles[metricTone(metric)]}>{formatValue(metric.value, metric.unit)}</strong>
        </div>
      ))}
    </div>
  );
...
function Direction({ label, tone, values }: { label: string; tone: 'profit' | 'loss'; values: string[] }) {
  return (
    <p className={styles['review-directions']}>
      <span>{label}：</span>
      <strong className={styles[`review-direction-${tone}`]}>{values.length ? values.join('、') : '暂无数据'}</strong>
    </p>
  );
}
...
function EmptyState() {
  return <p className={styles['review-empty']}>暂无数据</p>;
}
function stars(score: TMarketReviewRating | null) {
  return score === null ? '暂不评分' : '★'.repeat(score) + '☆'.repeat(5 - score);
}
function formatValue(value: number | null, unit?: string) {
  return value === null
    ? '暂无数据'
    : `${unit === '%' && value > 0 ? '+' : ''}${value.toFixed(unit === '%' ? 2 : 0)}${unit ?? ''}`;
}
function formatPercent(value: number | null) {
  return formatValue(value, '%');
}
function formatMoney(value: number | null) {
  return value === null ? '暂无数据' : `${value.toFixed(2)}亿`;
}
