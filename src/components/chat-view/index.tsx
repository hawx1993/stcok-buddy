        ...
        <div className={cx(styles['trace-body'], styles['tool-call-body'])}>
          {toolCalls.map((toolCall) => {
            const duration = toolCall.endedAt
              ? new Date(toolCall.endedAt).getTime() - new Date(toolCall.startedAt).getTime()
              : undefined;
            return (
              <div className={styles['tool-call']} key={toolCall.id}>
                <div className={styles['tool-call-header']}>
                  <span>{toolCall.toolName}</span>
                  <span className={styles['tool-call-meta']}>
                    {toolCall.error ? '失败' : '成功'}
                    {duration !== undefined ? ` · ${duration}ms` : ''}
                  </span>
                </div>
                <div className={styles['tool-call-meta']}>
                  输入：{toolCall.inputSummary ?? summarizeInline(toolCall.input)}
                </div>
                {toolCall.error ? (
                  <div className={styles['tool-call-error']}>{toolCall.error}</div>
                ) : (
                  <div className={styles['tool-call-meta']}>
                    输出：{toolCall.outputSummary ?? summarizeInline(toolCall.output)}
                  </div>
                )}
              </div>
            );
...
function getSingleAgentCommand(text: string) {
  const command = builtInSlashItems.find(
...
function isStockAnalysisQuery(text: string) {
  return (
    /^\s*(?:\d{6}|茅台|贵州茅台|五粮液|泸州老窖|洋河股份|宁德|宁德时代|宁王|招行|招商银行|比亚迪|中信证券)\s*$/.test(
      text,
    ) ||
    (/分析|诊股|个股分析|帮我看看|看看/.test(text) &&
      /\d{6}|茅台|贵州茅台|五粮液|泸州老窖|洋河股份|宁德|宁德时代|宁王|招行|招商银行|比亚迪|中信证券/.test(text))
  );
}
