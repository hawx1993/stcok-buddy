...
export function App() {
  ...
  useEffect(() => {
    ...
    const reportDuration = () => track('online_duration', { duration_seconds: Math.round((Date.now() - startedAt) / 1000) });
    const onVisibilityChange = () => { if (document.visibilityState === 'hidden') reportDuration(); };
    ...
    <div className={styles.app}>
      ...
      <div className={styles.body}>
        <div className={styles['left-tools']}>
          <button className={styles['collapse-btn']} onClick={() => { trackButtonClick('toggle_left_sidebar'); toggleLeftSidebar(); }} title={isLeftSidebarCollapsed ? '展开侧边栏' : '收起侧边栏'} type="button">
          ...
          <button className={styles['search-btn']} onClick={() => { trackButtonClick('toggle_search'); setSearchOpen((open) => !open); }} title="搜索" type="button" aria-label="搜索">
            <Search size={17} />
          </button>
        </div>
        ...
        <div className={cx(styles['right-wrapper'], isRightPanelCollapsed && styles.collapsed)}>
          <div className={styles['right-rail']}>
            <button className={cx(styles['rail-btn'], rightPanelTab === 'favorites' && !isRightPanelCollapsed && styles.active)} onClick={() => openRightRail('favorites')} type="button" title="收藏个股" aria-label="收藏个股">
              <Star size={18} />
            </button>
            <button className={cx(styles['rail-btn'], rightPanelTab === 'stock' && !isRightPanelCollapsed && styles.active)} onClick={() => openRightRail('stock')} type="button" title="个股详情" aria-label="个股详情">
              <LineChart size={18} />
            </button>
            <button className={cx(styles['rail-btn'], rightPanelTab === 'board' && !isRightPanelCollapsed && styles.active)} onClick={() => openRightRail('board')} type="button" title="板块详情" aria-label="板块详情">
              <Layers size={18} />
            </button>
            <button className={cx(styles['rail-btn'], rightPanelTab === 'surge' && !isRightPanelCollapsed && styles.active)} onClick={() => openRightRail('surge')} type="button" title="个股异动" aria-label="个股异动">
              <Activity size={18} />
            </button>
            <button className={cx(styles['rail-btn'], rightPanelTab === 'news' && !isRightPanelCollapsed && styles.active)} onClick={() => openRightRail('news')} type="button" title="热点新闻" aria-label="热点新闻">
              <Newspaper size={18} />
            </button>
          </div>
