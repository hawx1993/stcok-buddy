...
describe('股票服务 K 线工具', () => {
  ...
  it('解析东财 K 线文本和市场时间', () => {
    expect(parseEastmoneyKline('2026-07-31,10,11,12,9,100,2000,0,1.2,0.1,3.4')).toEqual(
      expect.objectContaining({
        time: '2026-07-31',
        open: 10,
        close: 11,
...
describe('股票服务板块和行工具', () => {
  ...
  it('按代码合并记录并忽略空字段', () => {
    expect(
      mergeByCode(
        [{ code: '600001', name: '旧', price: 10 }],
        [
          { code: '600001', name: '', price: 11 },
...
describe('股票服务行情行转换', () => {
  ...
  it('转换市场板块行并记录板块类型和真实成交额', () => {
    expect(toMarketBoardRow({ f12: 'bk1234', f14: '机器人', f3: '2.5', f6: '500', kind: 'concept' })).toEqual(
      expect.objectContaining({
        code: 'BK1234',
        name: '机器人',
        changePercent: 2.5,
