...
import { formatMoney, formatNumber, formatPercent, formatPercentPoints } from '../stock-detail/format.js';
...
async function enrichBoardConstituents(
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      changePercent:
        latest?.changePercent === undefined ? (row.changePercent ?? '--') : formatPercentPoints(latest.changePercent),
...
function toBoardConstituent(item: {
  ...
  return {
    ...
    changePercent:
      item.changePercent === null || item.changePercent === undefined ? '--' : formatPercentPoints(item.changePercent),
...
async function getLocalBoardDetail(symbol: string, fallbackName?: string): Promise<BoardDetail> {
  ...
  return {
    ...
    constituents: rows.slice(0, 80).map((item) => ({
      ...
      name: item.name,
      price: item.price ?? '--',
      changePercent: item.changePercent === undefined ? '--' : formatPercentPoints(item.changePercent),
