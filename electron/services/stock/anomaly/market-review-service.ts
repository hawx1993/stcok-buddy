...
import { getMarketPageSnapshot, listEastmoneySurgeByDate, listHotFocus } from '../stock-detail/stock-client.js';
...
export async function getMarketReview(): Promise<TMarketReviewReport> {
  ...
  return {
    ...
    sentimentScore,
    sentiment: [
      metric('涨停', limitUps.length, '家'),
      metric('跌停', limitDowns.length, '家'),
      metric('炸板', broken.length, '家'),
      metric('连板', consecutiveCount, '家'),
      metric('最高板', strongestHeight, '板'),
      metric('昨日涨停指数', null, '%'),
      metric('昨日连板指数', null, '%'),
    ],
    wealthEffect: [
      metric('平均涨幅', mean(changes), '%'),
      metric('上涨股票', rising.length, '家'),
      metric('下跌股票', falling.length, '家'),
      metric('涨幅中位数', median(changes), '%'),
      metric('跌幅中位数', median(falling), '%'),
    ],
    profitDirections: themes
      .filter((theme) => (theme.changePercent ?? 0) > 0)
      .slice(0, 4)
      .map((theme) => theme.name),
    lossDirections: sectors
      .filter((item) => parsePercent(item.changePercent) !== null && parsePercent(item.changePercent)! < 0)
      .slice(-4)
      .map(themeName),
    hotThemes: themes,
    leaders,
...
function buildThemes(
  ...
  return sectors.slice(0, 6).map((item, index) => {
    ...
    return {
      ...
      score,
      changePercent,
      limitUpCount: related.length || null,
      leaderName: leader?.name ?? null,
      leaderCode: leader?.code ?? null,
      leaderHeight: leader ? readHeight(leader.description) : null,
      mainNetInflow: parseMoney(flow?.amount),
      amount: parseMoney(item.amount),
      limitUpStocks: related.map((stock) => ({
        code: stock.code ?? '',
        name: stock.name ?? stock.title,
        height: readHeight(stock.description),
      })),
      coreStocks: [item, flow]
        .filter((value): value is HotFocusItem => Boolean(value))
        .map((stock) => ({
          code: stock.code ?? '',
          name: stock.name ?? stock.title,
          changePercent: parsePercent(stock.changePercent),
        })),
...
function buildLeaders(limitUps: HotFocusItem[]): IMarketReviewLeader[] {
  return [...limitUps]
    ...
    .map((item) => ({
      ...
      concepts: splitConcepts(item.description),
      height: readHeight(item.description),
      amount: parseAmountInYi(item.description),
      turnoverRate: parseTurnover(item.description),
      sealAmount: parseSealAmount(item.amount ?? item.description),
      changePercent: parsePercent(item.changePercent),
    }));
}
...
function buildNextDayFocus(
  ...
  return [
    watch(
      'leader',
      'leader',
      leader ? `观察 ${leader.name} 是否继续封板或维持高位承接` : '暂无可验证龙头，观察最高连板股的高位承接',
      leader?.height ?? null,
      'up',
      '板',
    ),
    watch(
      'theme',
      'theme',
      followUpTheme ? `观察 ${followUpTheme.name} 是否出现接力与扩散` : '暂无可验证次级热点，观察是否出现新的板块接力',
      followUpTheme?.changePercent ?? null,
      'neutral',
      '%',
    ),
    watch(
      'liquidity',
      'liquidity',
      marketAmount === null ? '暂无可验证成交额基准' : `观察全市场成交额是否超过今日 ${marketAmount.toFixed(0)} 亿`,
      marketAmount,
      'neutral',
      '亿',
    ),
    watch(
      'sentiment',
      'sentiment',
      `观察涨停家数是否超过 70 家（今日 ${limitUpCount} 家）`,
      limitUpCount,
      limitUpCount >= 70 ? 'up' : 'warn',
      '家',
    ),
    watch(
      'risk',
      'risk',
      brokenRate === null ? '暂无可验证炸板率基准' : `观察炸板率是否低于今日 ${brokenRate.toFixed(1)}%`,
      brokenRate,
      'down',
      '%',
    ),
...
function metric(label: string, value: number | null, unit: IMarketReviewMetric['unit']): IMarketReviewMetric {
...
function parseAmountInYi(value?: string): number | null {
  const match = value?.match(/成交额\s*([\d.]+)亿/);
  return match ? Number(match[1]) : null;
}
function parseSealAmount(value?: string): number | null {
  const match = value?.match(/封单\s*([\d.]+)亿/);
  return match ? Number(match[1]) : null;
}
function parseTurnover(value?: string): number | null {
  const match = value?.match(/换手\s*([\d.]+)%/);
  return match ? Number(match[1]) : null;
}
function readHeight(value?: string): number | null {
  const match = value?.match(/(\d+)连板/);
  return match ? Number(match[1]) : null;
}
function splitConcepts(value?: string) {
  return (value ?? '')
    .split('·')
    .map((item) => item.trim())
    .filter((item) => item && !/(连板|换手|封单|成交额|开板)/.test(item))
    .slice(0, 3);
}
...
function toRating(value: number): TMarketReviewRating {
  return Math.max(1, Math.min(5, Math.ceil(value))) as TMarketReviewRating;
}
function ratingFromRank(index: number, changePercent: number | null, flow?: HotFocusItem): TMarketReviewRating | null {
  if (changePercent === null && !flow) return null;
  return toRating(Math.max(1, 5 - index));
}
...
function buildDataGaps(input: { pools: HotFocusItem[]; sectors: HotFocusItem[]; flows: HotFocusItem[] }) {
  return [
    !input.pools.length ? '涨停/炸板/跌停池' : '',
    !input.sectors.length ? '板块排行' : '',
    !input.flows.length ? '板块资金流' : '',
    '昨日涨停指数与昨日连板指数',
  ].filter(Boolean);
}
