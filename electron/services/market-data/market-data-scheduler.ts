...
import { getMarketDataStats, initializeMarketDataStore } from './market-data-store.js';
...
async function shouldAutoSyncMarketData(now = new Date()) {
  ...
  if (!stats.dailyBarCount || !stats.latestTradeDate) return true;
  const latestDate = new Date(`${stats.latestTradeDate}T00:00:00+08:00`);
