...
function chipResult(concentration90: number, profitRatio: number): IChipDistributionResult {
  return {
    latest: {
      date: '2026-08-17',
      concentration90,
...
function chip(symbol: string, concentration90: number, profitRatio: number): StockChipCacheRecord {
  return {
    symbol,
    fetchedAt: '2026-08-17T09:30:00.000Z',
    data: chipResult(concentration90, profitRatio),
  };
}

function quoteFromRow(item: IAShareMarketCapSnapshotRow) {
  const totalMarketCap = item.totalMarketCap === undefined
    ? 50
...
describe('条件选股真实数据服务', () => {
  ...
  it('使用本轮 stock-sdk 全市场快照覆盖陈旧本地字段后筛选并批量回写', async () => {
    ...
    configureDefaults(
      [
        row({ symbol: '600010', name: '本地旧数据一', industry: '旧板块', totalMarketCap: 50, turnoverRate: 18, amount: 50_000 }),
        row({ symbol: '600011', name: '本地旧数据二', industry: '新能源', totalMarketCap: 150, turnoverRate: 5, amount: 10_000 }),
      ],
  ...
  it('板块范围只保留行业和概念涨幅前五的真实成分股', async () => {
    const boards: MarketBoardRecord[] = [
      { code: 'BK001', name: '行业一', kind: 'industry', changePercent: 6, source: 'duckdb', updatedAt: '2026-08-17T09:30:00.000Z' },
      { code: 'BK002', name: '概念二', kind: 'concept', changePercent: 5, source: 'duckdb', updatedAt: '2026-08-17T09:30:00.000Z' },
      { code: 'BK003', name: '行业三', kind: 'industry', changePercent: 4, source: 'duckdb', updatedAt: '2026-08-17T09:30:00.000Z' },
      { code: 'BK004', name: '概念四', kind: 'concept', changePercent: 3, source: 'duckdb', updatedAt: '2026-08-17T09:30:00.000Z' },
      { code: 'BK005', name: '行业五', kind: 'industry', changePercent: 2, source: 'duckdb', updatedAt: '2026-08-17T09:30:00.000Z' },
      { code: 'BK006', name: '概念六', kind: 'concept', changePercent: 1, source: 'duckdb', updatedAt: '2026-08-17T09:30:00.000Z' },
    ];
    configureDefaults(
      [
        row({ symbol: '600001', name: '前五板块成分股' }),
        row({ symbol: '600006', name: '第六板块成分股' }),
      ],
      {
        listMarketBoards: async () => boards,
        listBoardConstituents: async (boardCode) => {
          if (boardCode === 'BK001') {
            return [{ boardCode, stockCode: '600001', stockName: '前五板块成分股', position: 0, updatedAt: '2026-08-17T09:30:00.000Z' }];
          }
          if (boardCode === 'BK006') {
            return [{ boardCode, stockCode: '600006', stockName: '第六板块成分股', position: 0, updatedAt: '2026-08-17T09:30:00.000Z' }];
          }
          return [];
        },
      },
  ...
  it('本地快照为空时直接使用真实全市场行情创建候选并回写', async () => {
    ...
    expect(upsertSecurities).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ symbol: '600001' })]));
