...
export function createInitialAgentPlan(context: IAgentContext): IAgentPlan {
  ...
  const items = createPlanItems(context).map(
    (item): IAgentPlanItem => ({
      ...item,
...
export function formatPlanUpdatedMessage(plan: IAgentPlan): string {
  ...
  const changed = plan.items.filter(
    (item) => item.status === 'skipped' || item.status === 'blocked' || item.status === 'failed',
  );
  if (!gaps && !changed.length) return '分析计划状态已更新。';
  const changedText = changed.length
    ? `；已调整：${changed.map((item) => `${item.title}=${item.status}`).join('、')}`
    : '';
...
function createAnalysisPlanItems(context: IAgentContext): IPlanTemplateItem[] {
  ...
  if (asksSurgeReason) {
    return [
      ...
      item(
        'capital-flow',
        '核查资金流和特大单',
        '异动归因需要区分主力资金、主动买卖和盘口大单是否共振。',
        ['资金流', '特大单'],
        ['market-data', 'analysis-capital'],
        '资金流缺失时只保留量价和事件线索，不判断主力方向。',
      ),
      item(
        'theme-news',
        '核查题材、新闻与公告',
        '判断拉升是否有题材、公告或新闻催化，避免只凭涨幅归因。',
        ['新闻', '公告', '热点题材'],
        ['market-data', 'analysis-sentiment'],
      ),
      item(
        'technical-confirm',
        '检查短线技术结构',
        '确认异动是否突破关键位或只是冲高回落。',
        ['K线', '技术指标'],
        ['market-data', 'analysis-technical'],
      ),
      item(
        'risk-check',
        '排除事件和监管风险',
        '最终结论前需确认是否存在公告、减持、监管等未排除风险。',
        ['公告', '风险提示'],
        ['analysis-report'],
      ),
    ];
  ...
  if (asksRisk) {
    return [
      item(
        'quote-risk',
        '检查价格和成交异常',
        '先确认是否已有放量下跌、冲高回落或跌破关键位迹象。',
        ['实时行情', 'K线', '成交额'],
        ['quote', 'market-data', 'analysis-technical'],
      ),
      item(
        'capital-risk',
        '检查资金流出风险',
  ...
  const base = [
    item(
      'quote-strength',
      asksSelection ? '评估行情强度' : '确认行情基础',
      '先检查最新价格、涨跌幅、成交额和换手，判断是否具备关注基础。',
      ['实时行情', '成交额', '换手率'],
      ['quote', 'market-data'],
    ),
    item(
    ...
    item(
      'sector-news-risk',
      '核查板块强度、新闻公告和监管风险',
      '避免忽略题材退潮、公告利空、减持监管等事件风险。',
      ['行业/概念强度', '新闻', '公告'],
      ['market-data', 'analysis-fundamental', 'analysis-sentiment'],
    ),
    item(
      'history-similarity',
      '参考相似形态或历史表现',
      '结合历史日线观察类似量价结构后的短期表现，但不外推确定性结果。',
      ['历史日线'],
      ['market-data', 'analysis-technical'],
      '历史样本缺失时跳过相似形态判断。',
      true,
...
function technicalPlanItems(): IPlanTemplateItem[] {
  return [
    ...
    item(
      'kline-trend',
      '检查K线趋势和量价结构',
      '用历史K线判断趋势、放量缩量、突破或跌破。',
      ['K线', '成交量'],
      ['market-data', 'technical', 'analysis-technical'],
      'K线缺失时技术形态结论标记为不可判断。',
    ),
    item(
      'indicator-signal',
...
function themeAttributionPlanItems(): IPlanTemplateItem[] {
  return [
    item(
      ...
      '先确认热点样本来自真实异动数据。',
      ['强势股', '热点题材'],
      ['theme-attribution-data'],
    ),
    item(
      'theme-flow',
      '检查题材资金流和扩散',
      '题材归因需要看资金和个股扩散，而不是单只股票表现。',
      ['资金流', '板块'],
      ['theme-attribution-data'],
    ),
    item(
...
function createPlanSummary(intent: TAgentIntent, target?: string): string {
  ...
  if (intent === 'news-announcements')
    return `为了解读新闻公告影响，我会先核查${targetText}的真实新闻、公告和风险证据：`;
  ...
  if (intent === 'board' || intent === 'industry-ranking' || intent === 'hot-concepts')
    return '为了判断板块/题材强度，我会先核查排行、资金、成分扩散和风险：';
