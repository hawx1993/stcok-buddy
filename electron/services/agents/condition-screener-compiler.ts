...
interface ICommandSegmentResolution {
  correctedSegment?: string;
  correctionWarning?: string;
  error: string;
}

interface IParameterNameCandidate {
  parameter: IConditionScreenerParameter;
  distance: number;
}
...
function parseCommandSegments(text: string, state: IWorkingState): boolean {
  ...
  for (const segment of segments) {
    ...
    let appliedSegment = segment;
    let resolution: ICommandSegmentResolution | undefined;
    let applied = applyCommandSegment(segment, state);
    if (!applied) {
      state.lastKey = undefined;
      resolution = resolveCommandSegment(segment);
      if (resolution.correctedSegment) {
        appliedSegment = resolution.correctedSegment;
        applied = applyCommandSegment(appliedSegment, state);
      }
    }
    if (applied) {
      parsed = true;
      if (resolution?.correctionWarning) state.warnings.push(resolution.correctionWarning);
      const key = state.lastKey;
      if (key && commandKeys.has(key)) {
        state.errors.push(`参数重复：--${appliedSegment}。请只保留一个同类条件。`);
      }
      if (key) commandKeys.add(key);
    } else if (segment) {
      state.errors.push(resolution?.error ?? `未识别参数：--${segment}`);
    }
  }
  return parsed;
}
...
function applyCommandSegment(segment: string, state: IWorkingState): boolean {
  ...
  const marketScope = /^市场(?:范围)?=(.+)$/.exec(normalized);
  if (marketScope) return setMarketScopes(state, marketScope[1]);
...
function resolveCommandSegment(segment: string): ICommandSegmentResolution {
  const normalizedSegment = normalizeCommandSegment(segment);
  const parameterName = extractCommandParameterName(normalizedSegment);
  const normalizedName = normalizeParameterName(parameterName);
  if (!normalizedName) {
    return {
      error: `未能识别参数“--${segment}”。请输入 ${CONDITION_SCREENER_HELP_TRIGGER} 从参数弹窗选择。`,
    };
  }

  const candidates = findParameterNameCandidates(normalizedName);
  const allowedDistance = maxParameterNameDistance(normalizedName.length);
  const nearestDistance = candidates[0]?.distance;
  const nearestCandidates =
    nearestDistance === undefined || nearestDistance > allowedDistance
      ? []
      : candidates.filter((candidate) => candidate.distance === nearestDistance);

  if (!nearestCandidates.length) {
    return {
      error: `未能确定参数“--${segment}”对应哪个真实参数。请输入 ${CONDITION_SCREENER_HELP_TRIGGER} 从参数弹窗选择。`,
    };
  }

  if (nearestCandidates.length > 1) {
    const choices = nearestCandidates
      .map((candidate) => `${candidate.parameter.name}（${candidate.parameter.example}）`)
      .join('、');
    return {
      error: `参数“--${segment}”存在歧义，可能是：${choices}。请确认你想使用哪个参数？`,
    };
  }

  const [candidate] = nearestCandidates;
  const replacementName = extractCommandParameterName(candidate.parameter.example.slice(2));
  if (candidate.distance === 0 && normalizeParameterName(replacementName) === normalizedName) {
    return {
      error: `参数“${candidate.parameter.name}”的值或格式无法确认。请参考 ${candidate.parameter.example}，或输入 ${CONDITION_SCREENER_HELP_TRIGGER} 从参数弹窗选择。`,
    };
  }

  const correctedSegment = `${replacementName}${normalizedSegment.slice(parameterName.length)}`;
  return {
    correctedSegment,
    correctionWarning: `已将参数“--${segment}”识别为“--${correctedSegment}”。`,
    error: `参数名称可能是“${candidate.parameter.name}”，但值或格式无法确认。请参考 ${candidate.parameter.example}，或输入 ${CONDITION_SCREENER_HELP_TRIGGER} 从参数弹窗选择。`,
  };
}

const CONDITION_SCREENER_HELP_TRIGGER = '`/条件选股 --`';

function findParameterNameCandidates(parameterName: string): IParameterNameCandidate[] {
  const candidatesByParameter = new Map<string, IParameterNameCandidate>();
  for (const parameter of conditionScreenerParameters) {
    for (const alias of parameter.aliases) {
      const candidate: IParameterNameCandidate = {
        parameter,
        distance: damerauLevenshteinDistance(parameterName, normalizeParameterName(alias)),
      };
      const current = candidatesByParameter.get(parameter.id);
      if (!current || candidate.distance < current.distance) candidatesByParameter.set(parameter.id, candidate);
    }
  }
  return [...candidatesByParameter.values()].sort(
    (left, right) => left.distance - right.distance || left.parameter.name.localeCompare(right.parameter.name, 'zh-CN'),
  );
}

function extractCommandParameterName(segment: string): string {
  const operatorIndex = segment.search(/[=<>]/);
  if (operatorIndex >= 0) return segment.slice(0, operatorIndex);
  if (/^(?:返回|展示)/.test(segment)) return segment.replace(/\d.*$/, '');
  return segment;
}
...
function maxParameterNameDistance(length: number): number {
  if (length <= 2) return 0;
  return length <= 5 ? 1 : 2;
}

function damerauLevenshteinDistance(leftValue: string, rightValue: string): number {
...
function setMarketScopes(state: IWorkingState, rawScopes: string): boolean {
  const values = rawScopes.split(/[,，、+|/]/).filter(Boolean);
  const scopes = values.map(marketScopeFromValue);
  if (!scopes.length || scopes.some((scope) => scope === undefined)) return false;
  const uniqueScopes = unique(scopes.filter((scope): scope is TConditionScreenerMarketScope => Boolean(scope)));
  state.input.marketScopes = uniqueScopes;
  setCriterion(state, 'market-scope', `市场范围：${uniqueScopes.map(marketScopeLabel).join('、')}`);
  return true;
}
...
function marketScopeFromValue(value: string): TConditionScreenerMarketScope | undefined {
  if (/^(沪市|上证|沪股|沪A)$/i.test(value)) return 'sh';
  if (/^(深市|深证|深股|深A)$/i.test(value)) return 'sz';
  if (value === '北交所' || value === '北证') return 'bj';
  if (value === '科创板') return 'kc';
  if (value === '创业板') return 'cy';
  if (value === '主板') return 'main';
  return undefined;
}
