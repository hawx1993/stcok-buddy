...
import { getLatestDailyBar, listDailyBars, listLatestMarketRows } from '../../stock-db/market-data-store.js';
...
export const queryLocalDuckDBData: AgentTool<{ symbol?: string; limit?: number }, ILocalDuckDBQueryResult> = {
  ...
  async run(input) {
    ...
    if (symbol) {
      ...
      if (latestBarResult.status === 'rejected')
        warnings.push(`本地 DuckDB 最近日线读取失败：${formatError(latestBarResult.reason)}`);
    ...
    if (statusResult.status === 'rejected')
      warnings.push(`本地 DuckDB 同步状态读取失败：${formatError(statusResult.reason)}`);
    if (rowsResult.status === 'rejected')
      warnings.push(`本地 DuckDB 市场快照读取失败：${formatError(rowsResult.reason)}`);
