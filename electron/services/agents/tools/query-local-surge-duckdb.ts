...
function matchesSurgeOrderSide(item: TSurgeOrderFields, side: TSurgeOrderSide) {
  ...
  return side === 'buy'
    ? /买入|大笔买入|特大单买入/.test(textValue) && !/卖出|大笔卖出|特大单卖出/.test(textValue)
    : /卖出|大笔卖出|特大单卖出/.test(textValue) && !/买入|大笔买入|特大单买入/.test(textValue);
}
function filterSurgeOrders<T extends TSurgeOrderFields>(rows: T[], filter: ISurgeOrderFilter) {
  return isSurgeOrderFilterActive(filter)
    ? rows.filter(
        (item) =>
          matchesSurgeOrderSide(item, filter.side) &&
          (filter.minHands === undefined || largeOrderHands(item) >= filter.minHands),
      )
    : rows;
}
...
function dedupeSurgeOrdersByStock<
  T extends { code?: string; id?: string; time?: string; changePercent?: string; tradeDate?: string },
>(rows: T[]): T[] {
  const best = new Map<string, T>();
  for (const row of rows) {
    if (!row.code) best.set(`__no-code-${best.size}`, row);
    else {
...
function localRows(rows: unknown[], warnings: string[]): ILocalQueryOutput {
  return {
    source: 'duckdb:surge',
    storage: 'local',
...
function emptyLocal(warnings: string[]): ILocalQueryOutput {
  return localRows([], warnings);
}
  ...
  async run(input) {
    ...
    try {
      ...
      if (isSurgeOrderFilterActive(orderFilter)) {
        ...
        const rows = dedupeSurgeOrdersByStock(filterSurgeOrders(allRows, orderFilter)).slice(
          0,
          limit(record, 100, 1000),
        );
        return localRows(
          rows,
          rows.length
      ...
      return {
        ...
        dataset: 'stock_surge_events',
        dates,
        warnings: dates.length ? warnings : ['本地 surge DuckDB 暂无异动历史日期'],
