...
import { getMarketDataStats, listLatestMarketRows, listStockChips } from '../../stock-db/market-data-store.js';
...
function hasChipRangeCondition(options: IScreenLocalAStocksInput) {
  return (
    options.concentration90Min !== undefined ||
    options.concentration90Max !== undefined ||
    options.concentration70Min !== undefined ||
    options.concentration70Max !== undefined
  );
}
function hasChipCondition(options: IScreenLocalAStocksInput) {
  return (
    hasChipRangeCondition(options) ||
    options.profitRatioMin !== undefined ||
    (options.sortBy ?? 'changePercent') === 'concentration90'
  );
}
function recentChipWindow(result: IChipDistributionResult, options: IScreenLocalAStocksInput): ChipDistribution[] {
  ...
  return (options.chipMatchMode ?? 'latest') === 'latest'
    ? result.latest
      ? [result.latest]
      : []
    : result.distributions.slice(-days);
}
function chipItemPassesRanges(
  ...
  return (
    passesNumberRange(ratioPercent(item.concentration90), options.concentration90Min, options.concentration90Max) &&
    passesNumberRange(ratioPercent(item.concentration70), options.concentration70Min, options.concentration70Max)
  );
}
function countChipMatches(result: IChipDistributionResult, options: IScreenLocalAStocksInput) {
  const window = recentChipWindow(result, options);
  return hasChipRangeCondition(options)
    ? window.filter((item) => chipItemPassesRanges(item, options)).length
    : window.length;
}
function chipWindowPasses(result: IChipDistributionResult, options: IScreenLocalAStocksInput) {
  const mode = options.chipMatchMode ?? 'latest';
  const window = recentChipWindow(result, options);
  if (!hasChipRangeCondition(options)) return true;
  if (!window.length || (mode === 'all' && window.length < (options.chipLookbackDays ?? 1))) return false;
  const matches = countChipMatches(result, options);
  return mode === 'any' ? matches > 0 : mode === 'all' ? matches === window.length : matches > 0;
}
...
function buildScreenInput(input: unknown): IScreenLocalAStocksInput {
  ...
  return {
    ...
    sortBy: enumValue(
      record,
      'sortBy',
      ['changePercent', 'concentration90', 'amount', 'turnoverRate'] as const,
      'changePercent',
    ),
...
export const screenLocalAStocks: AgentTool<IScreenLocalAStocksInput, IScreenLocalAStocksOutput> = {
  ...
  description:
    'Screen all A-shares from local DuckDB by quote snapshot (changePercent/turnoverRate) and chip distribution conditions. Prefer for market-wide local screening.',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  async run(input) {
    ...
    if (marketRowsResult.status === 'rejected')
      warnings.push(`本地行情快照读取失败：${formatError(marketRowsResult.reason)}`);
    ...
    const needsChipCondition = hasChipCondition(options);
    ...
    const matched = marketRows
      ...
      .map((row): IScreenLocalAStockRow | undefined => {
        ...
        const chipData = chipRecord && isChipDistributionResult(chipRecord.data) ? chipRecord.data : undefined;
        if (needsChipCondition && !chipData) {
          if (!chipRecord) missingChipCount += 1;
          else missingChipConcentrationCount += 1;
          return undefined;
        }
        ...
        if (hasChipRangeCondition(options) && !chipData) return undefined;
        if (hasChipRangeCondition(options) && chipData && !chipWindowPasses(chipData, options)) return undefined;
        if (
          !passesNumberRange(row.changePercent, options.changePercentMin, options.changePercentMax) ||
          !passesNumberRange(row.turnoverRate, options.turnoverRateMin, options.turnoverRateMax) ||
          !passesNumberRange(profitRatioPercent, options.profitRatioMin, undefined)
        )
          return undefined;
        return {
          code: row.code,
          ...
          chipMatchedDays: chipData ? countChipMatches(chipData, options) : 0,
          recentConcentration90Percent: chipData ? windowPercents(chipData, 'concentration90', options) : [],
          recentConcentration70Percent: chipData ? windowPercents(chipData, 'concentration70', options) : [],
        };
      })
    ...
    if (!chips.length && needsChipCondition) warnings.push('本地 DuckDB 暂无可用筹码缓存数据');
    if (missingChipCount > 0) warnings.push(`${missingChipCount} 只股票缺少本地筹码缓存，未纳入筹码条件筛选`);
    if (missingChipConcentrationCount > 0)
      warnings.push(`${missingChipConcentrationCount} 只股票缺少有效筹码集中度，未纳入筹码条件筛选`);
