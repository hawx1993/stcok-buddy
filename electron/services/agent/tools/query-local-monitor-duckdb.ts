...
import {
  ...
  listMonitorDates,
  listMonitorHistory,
} from '../../stock/monitor-history-store.js';
    ...
    try {
      if (!date) {
        ...
        return {
          ...
          dataset: 'ai_monitor_events',
          dates,
          warnings: dates.length ? warnings : ['本地 monitor DuckDB 暂无监控历史日期'],
          isEmpty: dates.length === 0,
        };
      }
      const categories = monitorCategories(record);
      const rows = await listMonitorHistory({
        date,
        categories,
        offset: num(record, 'offset', 0),
        limit: limit(record, 50, 1000),
      });
