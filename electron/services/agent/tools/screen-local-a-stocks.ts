...
function hasChipCondition(options: IScreenLocalAStocksInput) { return hasChipRangeCondition(options) || options.profitRatioMin !== undefined || (options.sortBy ?? 'changePercent') === 'concentration90'; }
...
export const screenLocalAStocks: AgentTool<IScreenLocalAStocksInput, IScreenLocalAStocksOutput> = {
  ...
  description: 'Screen all A-shares from local DuckDB by quote snapshot (changePercent/turnoverRate) and chip distribution conditions. Prefer for market-wide local screening.',
  inputSchema: { type: 'object', properties: { changePercentMin: { type: 'number' }, changePercentMax: { type: 'number' }, turnoverRateMin: { type: 'number' }, turnoverRateMax: { type: 'number' }, concentration90Max: { type: 'number' }, concentration90Min: { type: 'number' }, concentration70Max: { type: 'number' }, concentration70Min: { type: 'number' }, profitRatioMin: { type: 'number' }, chipLookbackDays: { type: 'number' }, chipMatchMode: { type: 'string', enum: ['latest', 'all', 'any'] }, limit: { type: 'number' }, sortBy: { type: 'string', enum: ['changePercent', 'concentration90', 'amount', 'turnoverRate'] }, sortOrder: { type: 'string', enum: ['asc', 'desc'] }, includeST: { type: 'boolean' } } },
  async run(input) {
    ...
    const matched = marketRows.filter((row) => options.includeST || !/^\*?ST/i.test(row.name)).map((row): IScreenLocalAStockRow | undefined => {
      ...
      if (!passesNumberRange(row.changePercent, options.changePercentMin, options.changePercentMax) || !passesNumberRange(row.turnoverRate, options.turnoverRateMin, options.turnoverRateMax) || !passesNumberRange(profitRatioPercent, options.profitRatioMin, undefined)) return undefined;
      return { code: row.code, name: row.name, industry: row.industry, price: row.price, changePercent: row.changePercent, turnoverRate: row.turnoverRate, amount: row.amount, concentration90Percent, concentration70Percent, profitRatioPercent, chipDate: latest?.date, chipLookbackDays: options.chipLookbackDays, chipMatchedDays: chipData ? countChipMatches(chipData, options) : 0, recentConcentration90Percent: chipData ? windowPercents(chipData, 'concentration90', options) : [], recentConcentration70Percent: chipData ? windowPercents(chipData, 'concentration70', options) : [] };
