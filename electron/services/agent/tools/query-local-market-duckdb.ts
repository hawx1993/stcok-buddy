import { getStockChip, listBoardConstituents, listDailyBars, listLatestMarketRows, listMarketBoards, listSecurities, listStockChips, listStockSnapshots, listTradeCalendar, readBoardDetail, readBoardSnapshot, readDiscoverySnapshot } from '../../market-data/market-data-store.js';
...
type TMarketDataset = 'securities' | 'daily_bars' | 'trade_calendar' | 'market_rows' | 'market_boards' | 'board_constituents' | 'board_snapshot' | 'board_detail' | 'discovery_snapshot' | 'stock_chip' | 'stock_snapshot';
const DATASETS = ['securities', 'daily_bars', 'trade_calendar', 'market_rows', 'market_boards', 'board_constituents', 'board_snapshot', 'board_detail', 'discovery_snapshot', 'stock_chip', 'stock_snapshot'] as const;
...
function marketCapRow(row: { symbol: string; name: string; price?: number; changePercent?: number; turnoverRate?: number; amount?: number; totalMarketCap?: number; circulatingMarketCap?: number; pe?: number; pb?: number; fetchedAt?: string }) {
...
export const queryLocalMarketDuckDB: AgentTool<Record<string, unknown>, ILocalQueryOutput> = {
  ...
  description: 'Query whitelisted datasets from local stocksense-market DuckDB: securities, K-line, calendar, board caches, discovery snapshot, chips, stock snapshots.',
  ...
  async run(input) {
    ...
    try {
      switch (dataset) {
        ...
        case 'stock_snapshot': return localRows(dataset, (await listStockSnapshots(limit(record, 1000, 10000))).map(marketCapRow), warnings);
      }
    } catch (error) {
