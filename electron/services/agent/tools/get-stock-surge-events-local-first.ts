...
interface IStockSurgeEventsLocalFirstInput { symbol: string; days?: number; limit?: number; minHands?: number; }
interface IStockSurgeEventsLocalFirstOutput { source: 'right-panel-local-first' | 'a-stock-data'; storage: 'local' | 'remote'; symbol: string; rows: StockSurgeEvent[]; warnings: string[]; isEmpty: boolean; }
function tdxTransactionToSurgeEvent(symbol: string, row: ITdxTransactionRow, index: number): StockSurgeEvent {
  ...
  return { id: `tdx-transaction-${tradeDate}-${row.time || index}-${index}`, tradeDate, title: `${symbol} 逐笔成交`, code: symbol, time: row.time, price: row.price === null ? undefined : row.price, amount: hands === undefined ? undefined : `${side}${hands >= 10000 ? `${(hands / 10000).toFixed(2).replace(/\.00$/, '')}万手` : `${hands.toFixed(0)}手`}`, description: `通达信逐笔成交${tag}`, tag, type: side === '卖出' ? 'plummet' : side === '买入' ? 'surge' : 'neutral' };
}

/** 模型可调用：优先读取右侧栏同源的真实异动历史，缺失时查询 a-stock-data 通达信逐笔成交。 */
export const getStockSurgeEventsLocalFirst: AgentTool<IStockSurgeEventsLocalFirstInput, IStockSurgeEventsLocalFirstOutput> = {
  ...
  inputSchema: { type: 'object', properties: { symbol: { type: 'string' }, days: { type: 'number' }, limit: { type: 'number' }, minHands: { type: 'number' } }, required: ['symbol'] },
  async run(input) {
    ...
    try {
      ...
      if (rows.length) return { source: 'right-panel-local-first', storage: 'local', symbol, rows, warnings, isEmpty: false };
      warnings.push('右侧栏同源个股异动服务未返回最近异动历史');
    } catch (error) {
      warnings.push(`右侧栏同源个股异动服务读取失败：${formatError(error)}`);
    }
    try {
      const transactions = await runAStockDataFn<ITdxTransactionRow[]>('tdx_transactions', { code: symbol, min_hands: minHands, limit });
