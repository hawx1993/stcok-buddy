...
const slashCommands = [
  {
    ...
    usage: '请输入股票代码或股票名称，例如：/综合投研报告 中公教育',
  },
  {
    name: '/新闻公告',
    intent: 'news-announcements' as const,
    usage: '请输入股票代码或股票名称，例如：/新闻公告 000858',
  },
  {
    name: '/题材归因',
    intent: 'theme-attribution' as const,
    usage: '今天哪些股票走强，主要是什么题材',
    allowEmptyArgs: true,
  },
  ...
  { name: '/复盘今日行情', intent: 'market-review' as const, usage: '直接发送即可，系统将复盘最近可用交易日行情', allowEmptyArgs: true },
  {
    name: '/技术面分析',
    intent: 'analysis' as const,
    singleAgent: 'technical' as const,
    usage: '请输入股票代码或股票名称，例如：/技术面分析 000858',
  },
  {
    name: '/基本面分析',
    intent: 'analysis' as const,
    singleAgent: 'fundamental' as const,
    usage: '请输入股票代码或股票名称，例如：/基本面分析 000858',
  },
  {
    name: '/资金面分析',
    intent: 'analysis' as const,
    singleAgent: 'capital' as const,
    usage: '请输入股票代码或股票名称，例如：/资金面分析 000858',
  },
  {
    name: '/情绪面分析',
    intent: 'analysis' as const,
    singleAgent: 'sentiment' as const,
    usage: '请输入股票代码或股票名称，例如：/情绪面分析 000858',
  },
  {
    name: '/筹码分布',
    intent: 'analysis' as const,
    singleAgent: 'chip' as const,
...
export async function runOrchestrator(
  ...
  let symbol =
    typeof resolvedSymbol?.output === 'string'
      ? resolvedSymbol.output
      : (resolvedSymbol?.output as { symbol?: string; name?: string } | undefined)?.symbol;
  let stockName =
    typeof resolvedSymbol?.output === 'object'
      ? (resolvedSymbol.output as { name?: string } | undefined)?.name
      : undefined;
    ...
    command
      ? {
          ...
          command: {
            ...
            label: stockName ? `${stockName}（${symbol}）` : undefined,
          },
        }
      : {
          ...
          intent: {
            ...
            target: symbol ?? context.boardKeyword,
            mode: intent === 'analysis' ? '多 Agent 协同分析' : '单流程分析',
            label: stockName ? `${stockName}（${symbol}）` : undefined,
          },
        },
  );
  ...
  const analysisAgentNodes = nodes.filter((n) => n.id.startsWith('analysis-') && n.id !== 'analysis-report');
  ...
  const hasDagReportStep = nodes.some((n) => n.id === 'analysis-report');
  const planAgents = [
    ...(nodes.some((n) => n.id === 'quote')
      ? [{ id: 'data', agent: 'DataAgent', description: '获取实时行情与K线数据' }]
      : []),
    ...analysisAgentNodes.map((n) => ({
      id: n.id.replace('analysis-', ''),
      agent: n.agent,
      description: n.description,
    })),
    ...otherNodes.map((n) => ({ id: n.id, agent: n.agent, description: n.description })),
    ...(nodes.some((n) => n.id === 'analysis-report')
...
function themeAttributionToCard(
  surge: HotFocusItem[],
...
function newsAnnouncementsToCard(
